// /components/travel/ListHero.tsx
import React from "react";
import { Badge } from "@/components/ui/badge";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import type { Author, ListMetadata, Place } from "@/types/travel";
...
export const ListHero: React.FC<ListHeroProps> = ({ title, description, metadata, author, places, coverImage }) => {
  return (
    <div className="relative h-[50vh] min-h-[400px] bg-black">
      <img src={coverImage} alt={title} className="absolute inset-0 w-full h-full object-cover opacity-90" />
      ...
      <div className="absolute bottom-0 left-0 right-0 px-4 py-8">
        <div className="container max-w-screen-xl mx-auto">
          <div className="max-w-3xl space-y-4">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white [text-shadow:_2px_2px_8px_rgb(0_0_0_/_90%)]">{title}</h1>
            ...
            <div className="flex items-center gap-6 pt-2 bg-black/40 backdrop-blur-sm px-3 py-2 rounded-lg inline-flex">
              <div className="flex items-center gap-2">
                ...
                <div className="text-white [text-shadow:_1px_1px_2px_rgb(0_0_0_/_90%)]">
                  ...
                  <p className="text-sm text-white/90 font-medium">{author.location}</p>
                </div>
              </div>
              <div className="h-8 w-px bg-white/30" />
              <div className="flex items-center gap-4 text-white [text-shadow:_1px_1px_2px_rgb(0_0_0_/_90%)]">
                <span className="text-sm font-medium">{places.length} places</span>
                <span className="text-sm font-medium">Updated {new Date(metadata.updatedAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        </div>
