import type { TravelList, Place, RelatedList } from "@/types/travel";

// Helper function to create a full place object from basic place data
const createDetailedPlace = (basicPlace: { citySlug: string; name: string; country: string }): Place => {
  return {
...
const getPlaceDescription = (city: string): string => {
  const descriptions: Record<string, string> = {
    Copenhagen: "A family paradise with interactive museums, safe cycling paths, and the magical Tivoli Gardens.",
    Amsterdam: "Child-friendly museums, beautiful parks, and boat tours make this city perfect for young families.",
    Munich: "Home to the Deutsches Museum, English Garden, and numerous family-friendly beer gardens.",
    Portland: "A food lover's paradise with innovative restaurants, food carts, and local markets.",
    Berlin: "Vibrant nightlife, inclusive spaces, and historic LGBTQ+ neighborhoods.",
    Stockholm: "Beautiful winter wonderland with cozy cafes and festive markets.",
    Prague: "Magical winter atmosphere with historic architecture and warming traditional cuisine.",
    Barcelona: "Budget-friendly hostels, free walking tours, and amazing street food.",
...
const generateRelatedLists = (mainTag: string): RelatedList[] => {
  return [
    {
      id: `${mainTag}-1`,
      title: `More ${mainTag.charAt(0).toUpperCase() + mainTag.slice(1)} Destinations`,
...
export const MOCK_LISTS: MockDataTravelList[] = [
  ...
  },
  {
    ...
    places: [
      ...
      { citySlug: "copenhagen-denmark", name: "Copenhagen", country: "Denmark" },
      { citySlug: "prague-czech-republic", name: "Prague", country: "Czech Republic" },
    ],
  ...
  },
  {
    ...
    places: [
      ...
      { citySlug: "cesky-krumlov-czech-republic", name: "Cesky Krumlov", country: "Poland" },
    ],
...
export const DETAILED_TRAVEL_LISTS: Record<string, TravelList> = Object.fromEntries(
