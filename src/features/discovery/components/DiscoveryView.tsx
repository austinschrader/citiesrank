...
import { MOCK_DISCOVERIES, ACTIVE_CHALLENGES, MOCK_USER } from '../data/mockData';
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Level {MOCK_USER.level} Explorer</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm text-muted-foreground">Next Level: {MOCK_USER.nextLevel.title}</p>
                <p className="text-sm font-medium">{MOCK_USER.points} / {MOCK_USER.nextLevel.points} XP</p>
              </div>
            </div>
            <Progress 
          ...
          <div className={cn(
            ...
            <div className="bg-accent rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-muted-foreground">{ACTIVE_CHALLENGES[0].timeLeft} left</span>
              </div>
              <p className="text-sm text-muted-foreground mb-3">{ACTIVE_CHALLENGES[0].description}</p>
              <Progress 
                value={(ACTIVE_CHALLENGES[0].progress.current / ACTIVE_CHALLENGES[0].progress.total) * 100} 
                className="h-2 mb-2" 
              />
              <div className="flex items-center justify-between text-sm">
                <span>{ACTIVE_CHALLENGES[0].progress.current}/{ACTIVE_CHALLENGES[0].progress.total} completed</span>
                ...
                <div className="relative aspect-square">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div>
                        <h3 className="text-white font-medium">{discovery.title}</h3>
                        <p className="text-white/80 text-sm">by {discovery.user?.name || 'Unknown'}</p>
                      </div>
                    </div>
                    {discovery.type === 'challenge' && discovery.progress && (
                      <div className="mt-2">
                        <Progress 
                          value={(discovery.progress.current / discovery.progress.total) * 100} 
                          className="h-1.5" 
                        />
                      </div>
                    )}
      ...
      <div className="w-80 border-l bg-card p-4 overflow-auto hidden lg:block">
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(MOCK_USER.stats).slice(0, 4).map(([key, value]) => (
              <div 
                key={key}
                className={cn(
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['#hiddengems', '#streetart', '#foodie', '#architecture', '#nature'].map(tag => (
              <Button 
                key={tag} 
                variant="outline" 
                size="sm"
