...
const StatsOverlay = ({ place }: { place: CitiesResponse }) => {
  return (
    <div 
      className="stats-overlay absolute inset-0 bg-gradient-to-br from-slate-900/95 to-slate-800/95 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 backdrop-blur-sm flex items-center justify-center"
    >
      <button
        onClick={(e) => {
          e.stopPropagation();
...
export const PlaceCardGrid = ({ places, className }: PlaceCardGridProps) => {
    ...
    >
      {places.map((place) => (
        <div
          ...
          onMouseLeave={(e) => {
            const overlay = e.currentTarget.querySelector('.stats-overlay');
            if (overlay) overlay.classList.remove('hidden');
          }}
        >
          <PlaceCard city={place} variant="basic" />
          <StatsOverlay place={place} />
        </div>
      ))}
    </div>
  );
